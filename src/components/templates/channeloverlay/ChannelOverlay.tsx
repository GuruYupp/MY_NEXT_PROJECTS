...
export default function ChannelOverlayTemplate(
  props: ChannelOverlayTemplateProps,
) {
      ...
      <div className={styles.channel_overlay_inner}>
        ...
        <div className={styles.channel_overlay_right_inner}>
          <div className={styles.meta}>
            {templateData.shareName && (
              <h2 className={styles.title}>{templateData.shareName}</h2>
            )}
          </div>
          <div className={styles.buttons}>
            {templateData.show_signin === "true" && (
              <Link href={`/${templateData.target_signin}`}>
                <span className={`${styles.btn} ${styles.signin_btn}`}>
                  {templateData.signin}
                </span>
              </Link>
            )}
            {templateData.show_favourite === "true" && (
              <Link href={`/${templateData.target_favourite}`}>
