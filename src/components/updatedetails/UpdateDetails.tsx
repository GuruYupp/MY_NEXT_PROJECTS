...
const UpdateDetails: FC<UpdatedetailsPropsType> = (props) => {
  ...
  return (
    <div className={`${styles.updatedetails_wrapper}`}>
      <div className={`${styles.updatedetails_container} ${(!showModal ? styles.show : styles.hide)}`}>
      <div className={`${styles.updatedetails_inner}`}>
        <span className={`${styles.updatedetails_close}`} onClick={closeModal}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            {updatetype === "email" && (
              <Fragment>
                ...
                />
                <UpdateDetailsInput control={control} name="newemail" defaultValue="" shouldUnregister={true} rules={{
          ...
          <Modal
            ...
            render={(modal: ModalType) => {
              function getModal() {
                ...
                switch (modal) {
                  case 'otpverify':
                    return <OtpVerify closeModal={handlecloseModal} sendDatatoComponent={getDataFromModal} verifydata={otpprops} backgroundnone={true}/>
                  default:
                    return <></>;
                }
              }
