...
function LinkWrapper({
  ...
  let targetpath = menu.targetPath.indexOf('http') > -1 ? menu.targetPath : `/${menu.targetPath}`
  return menu.subMenus.length === 0 ? (
    <Link href={targetpath}>{children}</Link>
  ) : (
    <span className={`${styles.more_menu}`}>
    { children }
    </span>
  );
}
...
function SubMenu({
  ...
  return <div className={`${styles.submenus}`}>
...
export default function Menu({ menu }: props) {
  ...
  const isActive: boolean =
    asPath == `/${menu.targetPath}` ||
      (asPath == "/" && menu.targetPath == "home") || checkPathinsubmenu()
      ? true
      : false;
  ...
  return (
    <LinkWrapper menu={menu}>
      <div className={`${styles.menu} ` + (isActive ? styles.active : "")} onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}>
        <span className={`${styles.displayText} ` }>{menu.displayText}</span>
        {
          (menu.subMenus.length > 0 && showsubMenu) &&  <SubMenu submenus={menu.subMenus}/>
        }
      </div>
    </LinkWrapper>
  );
