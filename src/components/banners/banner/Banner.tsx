...
const Banner:FC<bannerpropsType> = (props) => {
  const { banner, SlideIndex, activeSlideIndex,pauseSlider,playSlider } = props;
  ...
  return (
    <div className={styles.imageContainer}>
      <img
        src={`${appConfig.bannerImgpath}/content/banner/common/${banner.imageUrl.split(',')[1]
          }`}
        alt=""
      />
      {banner.params?.streamUrl && <div className={`${styles.playerContainer} ${(showplayer ? styles.show : '')}`}><div ref={playerRef}></div></div>}
    </div>
  )
}
