...
export default function SelectUserProfiles() {
  ...
  const handleImageonClick = (profile: subprofileInterface) => {
    postData('/service/api/auth/activate/user/profile', { "profileId": profile.profileId}).then((data)=>{
      if(data.status === true){
  ...
  return (
    <div className={`${styles.ProfilePage}`}>
      <div className={`${styles.profileContainer}`}>
        ...
        <Link href={`${'/profiles/manage-user-profile'}`}><button className={`${styles.profiles_btn}`}>Manage Profiles</button></Link>
      </div>
    </div>
  )
}