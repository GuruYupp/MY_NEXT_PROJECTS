...
import { TitleRestrictionsInterface, viewRestrictionInterface } from "../viewRestrictiontypes";
import appConfig from "@/app.config";
import { debunceFunction } from "@/utils";

const TitleRestrictions: FC<TitleRestrictionsInterface> = (props) => {
  const { blockedContents, handleSearch, queryContents,handleSuggestion,handleRemoveContent } = props;
  ...
  const suggestionClick = (suggestion:viewRestrictionInterface['queryContents'][0]) =>{
    handleSuggestion({id:suggestion.id,name:suggestion.name,category:suggestion.elemSubType})
  }
    ...
    <div className={`${styles.Container}`}>
      ...
      <div className={`${styles.suggestionsContainer}`}>
        {queryContents?.map((content, keyindex) => {
          return (
            <p className={`${styles.suggestion}`} key={keyindex} onClick={()=>suggestionClick(content)}>
              {content.name}
            </p>
          );
        })}
          ...
          return Object.keys(itemsMap).map((itemkey:string, keyindex) => {
            return (
              <p className={`${styles.suggestion}`} key={keyindex}>
                {itemsMap[itemkey]}
                <span className={`${styles.close_icon}`} onClick={()=>contentClick(itemkey)}>
                  <img
